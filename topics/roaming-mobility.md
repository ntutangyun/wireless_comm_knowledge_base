...
Watch for: how EPCS context-transfer evolves through TGbn and which parts surface in observable IEs; MLO-roam mechanisms specifically (do you keep all setup links on roam, or only the primary?); ML / agent-driven predictive roaming benchmarked on real mobility traces; cross-references to `security-privacy` (the EPCS work touches authentication state and therefore has a security dimension too).
...
待关注：EPCS 上下文转移在 TGbn 中的演进——以及哪些部分会体现在可观察的 IE 中；专属 MLO 漫游机制（漫游时保留全部建立链路还是仅主链路？）；在真实移动轨迹上基准化的 ML / 智能体预测式漫游；与 `security-privacy` 的交叉（EPCS 工作触及认证状态，因此同时具有安全维度）。
