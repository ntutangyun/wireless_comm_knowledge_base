...
SIGCOMM 录用本身即是很强的质量信号（该会议每年收录的无线论文极少），且该主题使 Concord 直接落入本 KB 已有两个条目的问题空间：NSDI 26 的 Law 低时延链路层论文（2026-05-03_nsdi-law-802-11-low-latency-link-layer）和 BLADE 自适应竞争控制工作（2026-05-01_arxiv-blade-adaptive-contention）。后者证明了破坏实时流的是瞬态竞争区间膨胀而非平均 CSMA 效率。Concord 攻击的是同一尾时延问题的互补成因——突发长度侧——且来自 BLADE 背后同一个中科大竞争控制团队（Guo、Miao 两位作者同时出现在两篇论文中），表明这是一条有意延续的研究线：BLADE 处理竞争窗口维度，Concord 处理突发/TXOP 维度。
